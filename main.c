...
int main() {
  ...
  do {
    ...
    if (e == 1) {
      ...
      if(n!=0){
      fseek(arquivo, 0, SEEK_END);
      nregs = ftell(arquivo) / sizeof(c); // pega o tamanho do arquivo e divide pelo tamanho da struct para dar um valor aproximado ao que esta gravado no arquivo e não seu tamanho total
      fseek(arquivo, 0, SEEK_SET);
      ...
      else if(n==0) printf("arquivo vazio:\n");
    }// exclui
    else if (e == 4) {
      system("clear");
      ...
      fclose(arquivo);
        do{
          system("clear");
        printf("selecione uma das opções abaixo para a edição:\n\n1-marca\n2-modelo\n3-cor\n4-tipo\n5-placa\n6-ano de fabricação\n7-preço\n8-quilometragem\n9-editar tudo\n10-sair\nescreva:");
        scanf(" %i", &e);
        getchar();
        switch(e){
          case 1:{
...
vetor[reg].ano=ano;
      ...
      arquivo = fopen(nomearq, "wb"); // abrir o arquivo para escrita binaria
      if (arquivo == NULL) {
        printf("\nerro ao abrir o arquivo, pois ele não existe\n");
      }
